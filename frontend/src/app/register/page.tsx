...
export default function RegisterPage() {
  ...
  return (
    <div className="flex-grow flex items-center justify-center px-6 py-16 bg-slate-955 text-slate-200 min-h-screen relative">
      ...
      <div className="w-full max-w-2xl bg-slate-900 border border-slate-800 p-8 rounded-2xl flex flex-col gap-6 relative shadow-xl">
        ...
        {step === 1 && (
          <div className="flex flex-col gap-6">
            <div className="flex flex-col gap-1 text-center items-center">
              ...
              <h1 className="font-heading font-extrabold text-xl text-slate-50 tracking-tight">Select Workspace Type</h1>
              <p className="text-slate-400 text-xs mt-0.5 font-bold uppercase tracking-wider">Choose how you wish to collaborate on MahaCSR</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div 
                className={`p-6 rounded-2xl border cursor-pointer flex flex-col gap-4 transition-all ${
                  role === "NGO" 
                    ...
                    : "border-slate-800 bg-slate-900 hover:border-slate-850"
                }`}
                onClick={() => setRole("NGO")}
              >
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${
                  role === "NGO" ? "bg-indigo-100 text-indigo-700" : "bg-slate-955 text-slate-400"
                }`}>
                  <Landmark size={20} />
                </div>
                <div className="flex flex-col gap-1">
                  <span className="font-heading font-bold text-base text-slate-50">NGO Admin</span>
                  <span className="text-xs text-slate-300 leading-relaxed">Present project proposals, upload tax certifications, and raise milestone requests.</span>
                </div>
              </div>
              ...
              >
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${
                  role === "COMPANY" ? "bg-indigo-100 text-indigo-700" : "bg-slate-955 text-slate-400"
                }`}>
                  <Building2 size={20} />
                </div>
                <div className="flex flex-col gap-1">
                  <span className="font-heading font-bold text-base text-slate-50">Company Admin</span>
                  <span className="text-xs text-slate-300 leading-relaxed">Define CSR budget caps, browse project score matches, and release milestone payouts.</span>
                </div>
              </div>
            </div>
            ...
            <button 
              onClick={() => setStep(2)}
              className="w-full bg-indigo-700 hover:bg-indigo-650 text-white font-bold py-3.5 rounded-xl flex items-center justify-center gap-2 mt-2 transition-all shadow-sm"
            >
        ...
        {step === 2 && (
          <form onSubmit={handleRegister} className="flex flex-col gap-4">
            <div className="flex flex-col gap-1 text-center">
              <h1 className="font-heading font-extrabold text-xl text-slate-50 tracking-tight">Organization Profile</h1>
              <p className="text-slate-400 text-xs">Please fill the regulatory parameters for verification</p>
            </div>
